#' Computes the negative log-likelihood for the Segregation Selection model and initial fraction of 0.5
#'
#' Computes the negative log-likelihood for the Segregation Selection model, under binomial sampling 
#' Differs from the negllSS.flex.R function in that it assumes an known initial fraction of 0.5
#' See De Gelder et al 2004, 2008 and Ponciano et al 2007.
#' @param parm=c(lam,sig) which are the initial fraction of plasmid free cells, segregation rate and the plasmid cost. All three parameters must be specified according to the inverse transform of either the 1/(1+e^(-x)) or the -1/(1+e^(-x)) functions.
...
#' @export
#' @examples
#' negllSS.flex.nobeta0(parm, day.counts, nreps, gen,D1, neg.cost=TRUE)

negllSS.flex.nobeta0 <- function(parm, day.counts, nreps, gen,D1,neg.cost=TRUE){
	
	#Variable defs
	beta.o <- 0.5;
	lam	<- 1/(1+exp(-parm[1]));
	if(neg.cost==TRUE){sig <- -1/(1+exp(-parm[2]))}else{
		sig	<- exp(parm[2]);} 
