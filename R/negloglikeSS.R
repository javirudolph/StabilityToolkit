...
#' @param parm=c(beta.o,lam,sig) which are the initial fraction of plasmid free cells, segregation rate and the plasmid cost. All three parameters must be specified according to the inverse transform of either the 1/(1+e-x) or the exp(x) functions. See example below
...
negloglikeSS <- function(parm, day.counts, nreps, gen,D1){
	
	#Variable defs
	beta.o <-1/(1+exp(-parm[1]));#to transform between 0 and 1.
	lam	<- 1/(1+exp(-parm[2]));
	sig	<- exp(parm[3]); ###### remove EXP --go to line 489
